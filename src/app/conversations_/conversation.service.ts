...
export class ConversationsService {
  ...
  getMessages(ticketId: string) {
    return this.httpClient
      .get<Message[]>(
        `http://localhost:4201/api/comments/${ticketId}` + "?numOfComments=15"
      )
      .pipe(
        map(resData => {
          let messages = [];
  ...
  newMessage(data: Message) {
    return this.httpClient
      .post<{ comment_id: string }>("http://localhost:4201/api/comments/new", {
      ...
      .subscribe(resData => {
        ...
        this._conversationsArray.next(
          this._conversationsArray.value.concat(newMessage)
        );
      });
  }
  ...
  updateMessage(messageId: string, data: Message) {
    return this.httpClient
      .put(`http://localhost:4201/api/comments/${messageId}`, {
        _id: messageId,
        ticket_id: data.ticket_id,
        createdBy: data.createdBy,
        content: data.content,
      ...
      .pipe(
        ...
        tap(() => {
          ...
          this._conversationsArray.next(
            this._conversationsArray.value.concat(messageData)
          );
        })
      );
  }
